...
测试分三层：`test/*.test.ts` 是单元测试；`test/*-integration.test.mjs` 会**真的启动 `bin/open-bridge.js` 并走 HTTP**（外壳、鉴权闸门、两代 MCP 协议、多实例）；UI 测试必须和组件放在一起，即 `ui/src/**` 之下——放进 `test/` 会被 vitest 静默跳过。

其中鉴权闸门与协议不变量两份套件比周围的代码都老：每一条断言都是某次真实事故换来的，改之前请当作事故报告读。
