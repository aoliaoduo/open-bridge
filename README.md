...
| 模式 | 行为 |
| --- | --- |
| 频繁（默认） | 任务清单每勾选完一条，手机收到一条完成通知（服务端在 `set_todos` 落盘时自动推，不靠 AI 自觉）；AI 也可手动汇报 |
| 免打扰 | 只推「需要人回电脑前」的事件：需要选择/回复（`attention`）、对话结束（`finished`）；进展类一律静默 |

- **AI 可自选 Bark 参数**：notify 支持 `sound`（铃声）、`level`（`timeSensitive` 可穿透专注模式）、`call: 1`（持续响铃）、`badge`、`url`（点击跳转）；无反应监视的推送固定用 `timeSensitive`。
- **无反应监视**：连接静默超过设置页的「无反应提醒」分钟数（默认 60，0 = 关）且清单还有未完成项时，服务端自己推一条——网页 AI 标签页崩了、被限流卡死时唯一能叫回人的通道。
- **无反应监视**：连接静默超过设置页的「无反应提醒」分钟数（默认 60，0 = 关）且清单还有未完成项时，服务端自己推一条——网页 AI 标签页崩了、被限流卡死时唯一能叫回人的通道。
