...
| 出口 | 地址 | 说明 |
| --- | --- | --- |
| MCP | `/mcp/<路由令牌>` | Streamable HTTP MCP（38 个工具，两代协议同端点） |
...
例如 `"C:\Users\aolia\Desktop\aoliaoduo"` 或 `C:\Users\aolia\Desktop\aoliaoduo`），这个目录就是 AI
...
共 38 个定义，按配置档过滤后对外；旧工具名仍然可用，见 `docs/tools.md`），并去掉只在编辑器里有意义的壳（webview HTML、命令面板、`autoStart` 等），把宿主换成 CLI + 浏览器控制台。
