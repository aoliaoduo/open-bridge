...
test("only waiting and finished are accepted notification events", () => {
  ...
  assert.deepEqual(notifyToolArgKeys(), ["event", "title", "message"]);
});
...
test("one persistent Bark alert never schedules a second network push in the same episode", async () => {
  ...
  assert.equal(notificationEpisodeIsAlerted(), true);
...
test("finish fallback waits ten quiet minutes, not a todo completion, and never duplicates a self notice", () => {
  const base = {
    canSpeak: true,
    nowMs: 700_000,
    lastUsedMs: 100_000,
    ...
    hasTodos: true,
    allCompleted: true,
    completedAtMs: 54_000,
    notifiedSinceMs: 0,
    announcedForMs: 0,
  };
  assert.equal(finishNoticeVerdict(base), true);
  assert.equal(finishNoticeVerdict({ ...base, activeRequests: 1 }), false);
  assert.equal(finishNoticeVerdict({ ...base, allCompleted: false }), true, "an unfinished list must not block an ended conversation notice");
