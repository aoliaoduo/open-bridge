...
const bundled = path.join(root, "vendor", "rg.exe");
const rg = await ripgrepAvailable("rg") ? "rg"
  : process.platform === "win32" && await ripgrepAvailable(bundled) ? bundled : undefined;
...
test("normalizeRgPath handles relative and Windows separators", () => {
  assert.equal(normalizeRgPath("./src/a.ts"), "src/a.ts");
  assert.equal(normalizeRgPath(".\\src\\a.ts"), "src/a.ts");
});

test("runRipgrep parses matches, honors include globs and assembles context", async t => {
  if (!rg) { t.skip("ripgrep is unavailable"); return; }
  ...
  try {
    ...
    const edge = await runRipgrep({ query: "bar", cwd: dir, includeGlobs: ["notes.txt"], contextLines: 2, executable: rg });
    assert.equal(edge.matches.length, 1);
    assert.deepEqual(edge.matches[0].context_before, []);
    assert.deepEqual(edge.matches[0].context_after, []);
    await writeFile(path.join(dir, "middle.txt"), "a\nb\nHIT\nd\ne\n", "utf8");
    const middle = await runRipgrep({ query: "HIT", cwd: dir, includeGlobs: ["middle.txt"], contextLines: 2, executable: rg });
    assert.equal(middle.matches.length, 1);
    assert.deepEqual(middle.matches[0].context_before, [{ line: 1, text: "a" }, { line: 2, text: "b" }]);
    assert.deepEqual(middle.matches[0].context_after, [{ line: 4, text: "d" }, { line: 5, text: "e" }]);
  } finally {
    removeTempDir(dir);
  }
});

test("runRipgrep stops early at maxResults instead of scanning everything", async t => {
  if (!rg) { t.skip("ripgrep is unavailable"); return; }
...
test("runRipgrep throws loudly on an invalid pattern instead of answering empty", async t => {
  if (!rg) { t.skip("ripgrep is unavailable"); return; }
 ...
 */
test("ripgrepPatternRejection names the constructs ripgrep really refuses", async t => {
  if (!rg) { t.skip("ripgrep is unavailable"); return; }
