...
- `arr.sort(...)[0]` 在过滤结果为空时是 `undefined`，`arr[i + 1]` 越界同理。根因是 `noUncheckedIndexedAccess` 没开（实测开启后 `tsc` 报 85 处，`patch.ts` 与 `safe-probe.ts` 占一半 —— 那是一次严格度迁移，不是修 bug）。
...
- `npm audit` 在当前 registry 上不可用（没实现 security 端点），别把它的失败当成依赖有问题。
