...
export const TOOL_DEFINITIONS = [
  ...
  { name: "review_changes", description: "One cumulative git diff since the last review, including edits, shell effects and later commits. Reports the current working tree separately. Needs Git; mark_reviewed (default true) advances baseline.", inputSchema: { type: "object", properties: { since: { type: "string", enum: ["last_shown", "workspace_open"], description: "Diff target: since the last review (default) or since the workspace's first review checkpoint." }, mark_reviewed: { type: "boolean", description: "Advance the review baseline to the current state (default true)." }, max_patch_bytes: { type: "number", description: "Patch size budget in bytes (default 65536; head+tail truncation)." } } } , outputSchema: { oneOf: [{ type: "object", required: ["available", "reason"], properties: { available: { type: "boolean", enum: [false] }, reason: { type: "string" } }, additionalProperties: false }, { type: "object", required: ["available", "review_ref", "since", "summary", "files", "patch", "patch_truncated", "working_tree"], properties: { available: { type: "boolean", enum: [true] }, review_ref: { type: "string" }, since: { type: "string", enum: ["workspace_open", "last_shown"] }, summary: { type: "object", required: ["files", "additions", "deletions"], properties: { files: { type: "number" }, additions: { type: "number" }, deletions: { type: "number" } } }, files: { type: "array", items: { type: "object" } }, patch: { type: "string" }, patch_truncated: { type: "boolean" }, working_tree: { type: "object", required: ["clean", "summary"], description: "Uncommitted changes in the workspace now, separate from the full checkpoint diff.", properties: { clean: { type: "boolean" }, summary: { type: "object", required: ["files", "additions", "deletions"], properties: { files: { type: "number" }, additions: { type: "number" }, deletions: { type: "number" } } } } }, baseline_advanced: { type: "boolean" }, note: { type: "string" } } }] } },
