...
export const TOOL_DEFINITIONS = [
  ...
  { name: "file_op", description: "One file-system operation: create_directory, copy, move or delete. copy and move take source/destination (overwrite optional); delete takes recursive.", inputSchema: { type: "object", oneOf: [{ required: ["op", "path"], properties: { op: { enum: ["create_directory"] } }, description: "Create a directory at path." }, { required: ["op", "source", "destination"], properties: { op: { enum: ["copy"] } }, description: "Copy source to destination." }, { required: ["op", "source", "destination"], properties: { op: { enum: ["move"] } }, description: "Move source to destination." }, { required: ["op", "path"], properties: { op: { enum: ["delete"] } }, description: "Delete path; recursive is optional." }], properties: { op: { type: "string", enum: ["create_directory", "copy", "move", "delete"] }, path: { type: "string" }, source: { type: "string" }, destination: { type: "string" }, overwrite: { type: "boolean" }, recursive: { type: "boolean" } } }, outputSchema: FILE_OP_OUTPUT_SCHEMA },
  { name: "apply_patch", description: "Apply a patch inline (patch) or from a file (patch_file), exactly one. Unified diff updates existing files; ShunCode blocks (Add/Update/Delete File) also create and delete files.", inputSchema: { type: "object", oneOf: [{ required: ["patch"], not: { required: ["patch_file"] }, description: "Apply an inline patch." }, { required: ["patch_file"], not: { required: ["patch"] }, description: "Apply the patch stored at a workspace path." }], properties: { patch: { type: "string" }, patch_file: { type: "string", description: "Workspace path to a patch file; mutually exclusive with patch." }, expected_sha256: { type: "object", additionalProperties: { type: "string" } } } } , outputSchema: { type: "object", required: ["applied", "files", "changes"], properties: { applied: { type: "boolean", enum: [true] }, files: { type: "array", items: { type: "string" } }, changes: { type: "array", items: { type: "object", required: ["path", "action", "additions", "deletions", "diff"], properties: { path: { type: "string" }, action: { type: "string", enum: ["add", "update", "delete"] }, additions: { type: "number" }, deletions: { type: "number" }, diff: { type: "string" } } } } } } },
  { name: "review_changes", description: "One cumulative git diff of everything changed since the last review (edits plus shell side effects). Needs a Git workspace with a commit. mark_reviewed (default true) advances the baseline.", inputSchema: { type: "object", properties: { since: { type: "string", enum: ["last_shown", "workspace_open"], description: "Diff target: since the last review (default) or since the workspace's first review checkpoint." }, mark_reviewed: { type: "boolean", description: "Advance the review baseline to the current state (default true)." }, max_patch_bytes: { type: "number", description: "Patch size budget in bytes (default 65536; head+tail truncation)." } } } , outputSchema: { oneOf: [{ type: "object", required: ["available", "reason"], properties: { available: { type: "boolean", enum: [false] }, reason: { type: "string" } }, additionalProperties: false }, { type: "object", required: ["available", "review_ref", "since", "summary", "files", "patch", "patch_truncated"], properties: { available: { type: "boolean", enum: [true] }, review_ref: { type: "string" }, since: { type: "string", enum: ["workspace_open", "last_shown"] }, summary: { type: "object", required: ["files", "additions", "deletions"], properties: { files: { type: "number" }, additions: { type: "number" }, deletions: { type: "number" } } }, files: { type: "array", items: { type: "object" } }, patch: { type: "string" }, patch_truncated: { type: "boolean" }, baseline_advanced: { type: "boolean" }, note: { type: "string" } } }] } },
