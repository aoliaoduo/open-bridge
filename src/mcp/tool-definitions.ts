...
const SUPERVISED_PROCESS_LAUNCH_OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    ready: { type: "boolean" }, ready_checked: { type: "boolean" }, ready_error: { type: "string", description: "Present when the readiness check itself failed (for example a pattern that exceeded its evaluation budget): the process is still running and the command_id is the handle." }, restart_count: { type: "number" },
...
const READ_FILE_ROW_SCHEMA = {
  oneOf: [
    ...
    {
      type: "object",
      required: ["path", "content", "encoding", "sha256", "truncated", "bytes_returned", "bytes_total"],
      properties: {
        path: { type: "string" }, content: { type: "string" }, encoding: { type: "string", enum: ["utf8", "base64"] },
        binary: { type: "boolean", description: "True when automatic binary detection selected base64." },
        sha256: { type: ["string", "null"], description: "Whole-file digest only when this row read the complete file." },
        truncated: { type: "boolean", description: "True when this row omitted file data." },
        bytes_returned: { type: "number" }, bytes_total: { type: "number" },
        offset: { type: "number", description: "Actual byte offset of a base64 page." },
        next_offset: { type: ["number", "null"], description: "Pass as offset with encoding=base64 to continue a truncated base64 page; null when final or no-progress." },
        lines_returned: { type: "number" }, start_line: { type: "number" }, end_line: { type: "number" },
        lines_total: { type: ["number", "null"], description: "Known only when the text stream reached EOF." },
        next_start_line: { type: ["number", "null"], description: "Pass as start_line to continue after a newline-ended truncated text page; null means retry with a larger max_bytes." },
      },
    },
  ],
} as const;
...
export const TOOL_DEFINITIONS = [
  ...
  { name: "search_files", description: "Search files with regex (or literal regex=false); include/context and offset paging supported. truncated=more rows; partial=unreadable paths—fix access or narrow scope, then retry.", inputSchema: { type: "object", required: ["query"], properties: { query: { type: "string", description: "Regex pattern to search for, or literal text when regex is false." }, path: { type: "string", description: "File or directory scope for the search; defaults to the workspace." }, max_results: { type: "number", description: "Maximum matches to return in this page before reporting truncation." }, regex: { type: "boolean", description: "Regex by default; false for literal text." }, include: { type: "array", items: { type: "string" }, description: "Glob patterns of files to search, e.g. [\"*.ts\"]." }, offset: { type: "number", description: "Number of matches to skip before collecting (pagination); pair with max_results to page through large result sets." }, context: { type: "number", description: "Lines of context to return before/after each match (clamped to 0-20, default 0)." } } } , outputSchema: { type: "object", required: ["items", "truncated", "next_offset", "partial"], properties: { items: { type: "array", items: { type: "object", required: ["path", "line", "text"], properties: { path: { type: "string" }, line: { type: "number" }, text: { type: "string" }, context_before: { type: "array" }, context_after: { type: "array" } } } }, truncated: { type: "boolean", description: "True when more matches exist beyond this page; page with offset." }, next_offset: { type: ["number", "null"], description: "Pass as offset to continue; null means this page is final." }, partial: { type: "boolean", description: "True when unreadable or errored paths made this search non-exhaustive. Fix access or narrow to readable paths and retry; this is independent of page truncation." } } } },
  { name: "read_files", description: "Read files. Text pages return next_start_line; base64 pages return offset/next_offset. A byte-cap truncation without a cursor needs larger max_bytes. sha256 is whole-file digest or null.", inputSchema: { type: "object", required: ["paths"], properties: { paths: { type: "array", description: "One or more file paths to read, in the order their results should be returned; at most 20 per call.", maxItems: 20, items: { type: "string" } }, max_bytes: { type: "number", description: "Maximum bytes to read from each requested file." }, offset: { type: "number", description: "0-based byte offset for encoding=base64 only; pass next_offset to continue a base64 page." }, start_line: { type: "number", description: "First line to return, 1-based." }, end_line: { type: "number", description: "Last line to return, 1-based; inclusive." }, encoding: { type: "string", enum: ["utf8", "base64"], description: "Set base64 to read binary files; content is returned base64-encoded." } } } , outputSchema: ARRAY_RESULT_SCHEMA(READ_FILE_ROW_SCHEMA, "One entry per requested path, in the order asked. An unreadable path still gets an entry so items always lines up with the request.") },
  ...
  { name: "batch", description: "Run 1-20 tool calls in one roundtrip. sequential (default) or parallel; fail_fast stops sequential work. Results include submitted index, so repeated tools and an unrun tail are identifiable.", inputSchema: { type: "object", required: ["calls"], properties: { calls: { type: "array", description: "One to twenty calls, each with an advertised tool name and optional arguments.", minItems: 1, maxItems: 20, items: { type: "object", required: ["tool"], properties: { tool: { type: "string", description: "Canonical advertised tool name to call." }, arguments: { type: "object", description: "Arguments for that tool; omit when it takes none." } } } }, mode: { type: "string", default: "sequential", description: "sequential (default) runs calls in order; parallel starts them together.", enum: ["sequential", "parallel"] }, fail_fast: { type: "boolean", default: false, description: "Only for sequential batches: true stops after the first failure; false or omitted continues. Ignored in parallel." } } }, outputSchema: { type: "object", required: ["mode", "total", "succeeded", "failed", "stopped_early", "results"], properties: { mode: { type: "string", enum: ["sequential", "parallel"] }, total: { type: "number" }, succeeded: { type: "number" }, failed: { type: "number" }, stopped_early: { type: "boolean" }, results: { type: "array", items: { oneOf: [{ type: "object", required: ["index", "tool", "ok", "result"], properties: { index: { type: "number", description: "Zero-based position in the submitted calls array." }, tool: { type: "string" }, ok: { type: "boolean", enum: [true] }, result: {} } }, { type: "object", required: ["index", "tool", "ok", "error"], properties: { index: { type: "number", description: "Zero-based position in the submitted calls array." }, tool: { type: "string" }, ok: { type: "boolean", enum: [false] }, error: { type: "string" } } }] } } } } },
  { name: "run_script", description: "Compose workspace tools in JavaScript and return only what you need. Fresh scope; no fs/network. Failure or timeout returns bounded tool_outcomes; inspect any running call before retrying.", inputSchema: { type: "object", required: ["source"], properties: { source: { type: "string", description: "JavaScript with top-level await and return; a surrounding Markdown fence is tolerated." }, timeout_ms: { type: "number", description: "Wall-clock budget for the whole run in ms (default 30000, max 300000)." }, max_calls: { type: "number", description: "Cap on tool calls this script may make (default 60, max 200)." } } }, outputSchema: { type: "object", required: ["ok", "calls"], properties: { ok: { type: "boolean" }, result: {}, result_bytes: { type: "number" }, truncated: { type: "boolean" }, calls: { type: "number" }, by_tool: { type: "object" }, duration_ms: { type: "number" }, console: { type: "array", items: { type: "string" } }, tool_outcomes: { type: "array", description: "Present on a failed script: each already-started tool call, with a bounded recovery snapshot. status=running means inspect before retrying.", items: { type: "object", required: ["call_id", "tool", "status"], properties: { call_id: { type: "number" }, tool: { type: "string" }, status: { type: "string", enum: ["running", "succeeded", "failed"] }, result: {}, result_bytes: { type: "number" }, result_truncated: { type: "boolean" }, error: { type: "string" } } } }, phase: { type: "string" }, error: { type: "string" }, error_type: { type: "string" }, tool: { type: "string" }, line: { type: "number" }, code_preview: { type: "array", items: { type: "string" } }, hint: { type: "string" } } } },
