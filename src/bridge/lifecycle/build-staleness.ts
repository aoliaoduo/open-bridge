 ...
 * with. The module lives somewhere under `<repo>/dist/` when compiled (its exact
 * depth under it is an implementation detail the lookup must not depend on) and
 * under `<repo>/src/` under tsx, so the file extension is the honest discriminator — and it keeps dev mode out of this
...
function distDir(): string | undefined {
  ...
  // Walk up to the nearest `dist` ancestor: the module depth under it is an
  // implementation detail, and subdirectory regroups must not have to touch this.
