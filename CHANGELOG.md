...
- 修复 TUI 活动流「永远计时」：invoke 行与结局行的配对此前依赖双方都携带参数摘要，而结局行（由 MCP 端点记录）从不携带——配对永不成立，每个已完成的工具调用都显示为永久转圈的进行中，计时虚涨；`process · Started` 生命周期行则根本没有对应的结局行。现在结局行按工具名 FIFO 关闭最老的未闭合 invoke 并显示真实耗时；`process` 行改从命令表读取真实状态——存活进程照常计时，已结束进程显示真实存活时长（endedAt−startedAt），已清理的记录降级为中性标记、不虚构时间。仅影响显示，审计日志不变。- serve 终端新增 ainovel-cli 风格的 TUI 仪表盘（阶段一）：启动横幅之后进入备用屏幕，500ms 就地重绘——顶栏（版本 · 端口 · 隧道 · 工作区名 · 状态胶囊，繁忙时胶囊图标切换为实时 spinner）、概览卡（运行时长 / 调用 / 会话容量 / 进程 / 服务）、进程卡（运行中命令 + 输出缓冲水位条）、活动流（时间戳 + ✓/✕/spinner 三态图标；invoke 行在其结局落地时合并为携带真实耗时的一行，未观测到配对则不虚构耗时）、底部用量状态栏。健康度渐变沿用 ainovel-cli 的阈值（<70% 绿 / 70–84% 黄 / ≥85% 红），应用于会话容量与每命令输出缓冲占用。Ctrl+C 语义不变（无 raw mode），停止时恢复原始终端；无真实控制台（服务 / CI / 重定向）或 `--no-tui` 时保持原输出路径。渲染层为纯函数（CJK 感知列宽、精确几何、窄窗降级阶梯），由单测锁定。
