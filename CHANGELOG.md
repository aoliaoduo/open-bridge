...
- TUI 任务视图：按 Tab 在「活动 / 任务」间切换右侧宽面板（Esc 返回活动）。任务视图独占全宽，长标题按行宽自动折行完整显示，超长列表截断并提示；侧栏只保留一行「任务 N（M 进行中）」摘要，不再截断标题。任务与 Web 控制台同源（`set_todos`），重启后从持久化存储恢复。
- TUI 侧栏新增「变更」行：自最近一次 git commit 以来的工作区变更摘要（`+插入 -删除 · N 文件`，含未跟踪文件按整文件计行），每 5 秒异步刷新、渲染帧只读缓存，commit 后自动归零；非 git 仓库不显示。
