...
- **侧栏那条几乎没用的细滚动条。** `.sidebar` 高度锁死 `100vh`，九个导航项在稍矮的窗口里正好溢出一点点，于是常驻一条又窄又短的滚动条 —— 它在说「下面还有东西」，可实际只多出一行。滚动本身必须留着（窗口再矮就真的需要滚），改的是可见性：平时不占滚动槽也不显形，指针移进侧栏或键盘焦点进入时才出现。Firefox 走 `scrollbar-width`，Chrome / Edge 走 `::-webkit-scrollbar`。
