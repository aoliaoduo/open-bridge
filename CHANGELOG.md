...
- 修复 TUI 活动面板排序与跟随反转：事件数组被一次多余的翻转变成「最新在顶」，而跟随模式与 End 键均按「最新在底」设计——事件数一旦超过面板行数，跟随会锁死在最旧的事件窗口、新调用不再上屏，End 也指向最旧历史。现在面板按时间正序渲染（最新在底部）、跟随与 End 锁定最新事件；窄窗堆叠布局的活动流同样改为截取最新事件而非最旧。
