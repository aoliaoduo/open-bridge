...
## [Unreleased]
### Added
- **控制台能真正重启（并加载新构建）。** 之前那句建议（「停止」再「启动」）是做不到的，而且**即使做到了也没用**：停止会关掉承载控制台的监听器，页面随即失效；而同进程内的启停**不会重新加载代码**——Node 早就把旧模块装进内存了（生命周期里那个 stop→start 是给「接手空出的公网域名」用的，不是给更新用的）。现在运行控制里有两步确认的「重启」：实例先停、把 runtime 记录与启动锁清干净，再用**自己的命令行**拉起一个后继进程（同解释器、同 argv，不经 shell，所以带空格的路径不会坏），确认后继真的注册上来之后本体退出；后继没能起来时本体**回退到原地重启**，而不是留一个死端口。控制台在重启期间自己轮询 `/api/status`，一旦答应就自动重载页面（顺便拿到新构建注入的控制台令牌），全程不需要手动刷新；等待超时会明确告诉你改用一键启动脚本。**副作用写在明处**：网页发起的重启由**后台进程**承载（它必须比启动它的进程活得久），终端窗口不再持有它——停止请用控制台「退出进程」或 `open-bridge stop`。两处「构建比实例新」的提示也改成指向「重启」。
