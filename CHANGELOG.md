...
- **超大的超时值会变成「立刻超时」。** `setTimeout` 的延时存在 32 位有符号整数里，超过 2147483647 之后 Node 打一条 `TimeoutOverflowWarning` 然后**按 1 毫秒处理**。于是 `concurrency.holdTimeoutMs = 1e18`——意思明明是「这把锁基本上永远别自动释放」——实际变成「下一拍就释放」。症状离病因很远：一个资源在第一个调用还持有时被交给了第二个调用，而这正是 `resource_keys` 唯一要防的事。
...
- **`doctor` 把一个错了 5 小时半的时钟报成 `[OK]`。** 时区自动折算那轮只在 Windows 上验过，CI 一跑 Linux 就露了馅 —— 而两条失败各有各的原因，都不是翻译问题。

  其一，`normalizeTimezone()` 判「解析不出来」只认 `Etc/Unknown` 这一种拼法。那是 **Windows** 的 ICU 对 `TZ=CST-8` 的回答；**Linux** 对同一个值回答的是 `undefined`（取 `|| ""` 后是空串）。于是整个折算在 Linux 上从来没有触发过 —— 偏偏 CI 跑的就是 Linux。同一份「无法解析」的判断在 `local-commands.ts` 里写对了（两种都认），在 `node-host.ts` 里写漏了一种：**同一个概念在两处各写一遍，就是这么漂的**，现在提成一个 `unresolvable()` 内联判定。

  其二更严重，是个真的产品 bug：glibc 接受 `TZ=IST-5:30`（半小时偏移，折算逻辑刻意不碰），并把它解析成 **`UTC`** —— 一个完全合法的 IANA 名字。于是「按名字判断健康」的检查给出 `[OK]`，而进程的挂钟差了 5.5 小时。**这正是这套机制当初要解决的那个病**（`TZ` 无法识别 → 静默跑在 UTC → 日志时间全错），只不过换了个入口重新发作，而且这次伪装成了体检通过。

  判据因此从「zone 名字像不像话」改成「**它有没有兑现**」：`TZ` 设了个不是 UTC 的值、实际偏移却是 0，就是没兑现。刻意用「TZ 要的是什么」而不是「偏移是不是 0」来区分，否则**每一台真心跑在 UTC 的机器都会被报成坏的** —— 服务器和 CI runner 大多如此，包括抓到这个 bug 的那一台。`TZ=UTC` 仍然 `[OK]`，并补了一条测试钉住，因为这是新判据唯一可能引入的回归。报错文案也分开了：这一种不是「无法识别」，而是「识别了，但给的不是它要的」，顺带给出 `Asia/Kolkata` 这个真正该用的名字。
