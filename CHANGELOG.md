...
- TUI 地址下移至侧栏底部与工作台全屏纵向扩展：将原本位于屏幕最下方的控制台地址迁移至左侧栏最底部，移除易造成多行折行杂乱的超长 MCP 地址，仅保留 Web 控制台入口；取消宽屏工作台下占满全宽的独立底部页脚（Footer），使右侧活动/任务/变更面板多获得 2 行显示高度，视觉更紧凑自然。窄屏/极小窗口下依然保留底部页脚回退展示。
