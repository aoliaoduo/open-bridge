...
### Added
- **ngrok authtoken 可以在控制台里填了。** 以前隧道要跑起来，必须先在终端执行 `ngrok config add-authtoken <token>` —— 而「打开终端敲一条命令」恰恰是会卡住新手的那一步。更糟的是页面上「预留域名」就摆在那里，看上去配置已经齐了，于是人填完域名、开了隧道、拿到一条 ngrok 报错，却完全不知道缺的是凭据。现在设置 → 隧道的第一项就是 Authtoken，保存后重启隧道生效。
